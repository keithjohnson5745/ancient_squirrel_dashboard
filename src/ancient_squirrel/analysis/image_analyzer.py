            ...
            for i in range(0, len(valid_df), batch_size):
                ...
                for future in tqdm(as_completed(basic_futures), total=len(basic_futures), desc="Basic analysis"):
                    ...
                    try:
                        ...
                        if "error" not in result:
                            ...
                            result_df.at[idx, basic_analysis_col] = json.dumps(result)
                            analysis_results["successful"] += 1
                        else:
                            analysis_results["failed"] += 1
                    except Exception as e:
                        self.logger.error(f"Error processing basic analysis result: {e}")
                        analysis_results["failed"] += 1
                
                # Continuing the process method in image_analyzer.py
                ...
                if use_llm and self.llm_adapter:
                    ...
                    for idx, row in batch_df.iterrows():
                        ...
                        title = row[title_col] if title_col in row else None
                    ...
                    for future in tqdm(as_completed(llm_futures), total=len(llm_futures), desc="LLM analysis"):
                        ...
                        try:
                            ...
                            result_df.at[idx, llm_analysis_col] = json.dumps(result)
                            
                        except Exception as e:
                            self.logger.error(f"Error processing LLM analysis result: {e}")
