        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            for i in range(0, len(valid_df), batch_size):
                ...
                for idx, row in batch_df.iterrows():
                    ...
                    # Skip if invalid
                    if not title or not thumbnail_path or not os.path.exists(thumbnail_path):
                        continue
                    
                    # Get video ID if available
                    video_id = row[video_id_col] if video_id_col in row else None
                ...
                for future in tqdm(as_completed(basic_futures), total=len(basic_futures), desc="Joint analysis"):
                    ...
                    try:
                        ...
                        if "error" not in result:
                            ...
                            result_df.at[idx, joint_analysis_col] = json.dumps(result)
                            analysis_results["successful"] += 1
                        else:
                            analysis_results["failed"] += 1
                    except Exception as e:
                        self.logger.error(f"Error processing joint analysis result: {e}")
                        analysis_results["failed"] += 1
                ...
                if use_llm and self.llm_adapter:
                    ...
                    for future in tqdm(as_completed(llm_futures), total=len(llm_futures), desc="LLM joint analysis"):
                        ...
                        try:
                            ...
                            result_df.at[idx, llm_joint_analysis_col] = json.dumps(result)
                            
                        except Exception as e:
                            self.logger.error(f"Error processing LLM joint analysis result: {e}")
